'''
@ Summary: 函数img_preprocess对图片进行缩放尺寸为(224, 224)，转为int8类型,
...
def img_preprocess(img_path, dataset_h, img_size):
    ...
    with open(dataset_h, "w+") as f:
        if img_size[1] == 112:
            print("#define %s "%label + "{" + ", ".join(img_context) + "};", file=f)
        else:
...
def main():
    img_path = r'./test_imgs/bigspot1.jpg'

    model_input_txt = r'./model_input.txt'
    lcd_show_txt = r'./lcd_show.txt'

    img_input_size = (112, 112)  # 为模型输入图片尺寸
